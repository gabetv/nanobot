...
function removeFromInventory(itemId) { 
    ...
    if (index > -1) { 
        ...
        // Ne pas appeler updateInventoryDisplay ici si on en supprime plusieurs pour un coût
        // L'appelant (ex: upgradeNanobotModule) fera la mise à jour globale.
        return true; 
    } 
    return false; 
}
...
function updateShopDisplay() { 
    ...
    gameState.shopStock.forEach(itemId => { 
        ...
        if (!isPurchased) { 
            for (const resource in item.cost) { 
                if (gameState.resources[resource] < item.cost[resource]) { 
        ...
        let content = `<div class="item-details">
                        <h4 class="item-name text-lg ${isPurchased ? 'text-gray-500' : 'text-blue-300'}">${item.name} ${isPurchased ? '(Acquis)' : ''}</h4>
                        <p class="item-stats text-sm text-gray-400">${item.description}</p>`; 
        if (item.statBoost) { 
            content += `<p class="item-stats text-sm ${isPurchased ? 'text-gray-500' : 'text-green-300'}">Effets: ${Object.entries(item.statBoost).map(([s,v]) => `${s.charAt(0).toUpperCase()+s.slice(1)}: ${v > 0 ? '+' : ''}${v}`).join(', ')}</p>`; 
        } 
        content += `<p class="text-sm ${isPurchased ? 'text-gray-500' : 'text-yellow-400'} mt-1">Coût: ${costString}</p>
                    </div>`; 
        
        if (!isPurchased) {
            content += `<button class="btn ${canAfford ? 'btn-success' : 'btn-disabled'} btn-sm mt-2" onclick="buyItem('${item.id}')" ${!canAfford ? 'disabled' : ''}>Acheter</button>`;
        } else {
            content += `<button class="btn btn-disabled btn-sm mt-2" disabled>Déjà Acquis</button>`;
        }
    ...
    if(displayedItems === 0 && shopItemsListEl.innerHTML === ""){ 
         shopItemsListEl.innerHTML = "<p class='text-gray-500 italic'>Tous les articles disponibles ont été acquis.</p>";
    } else if (gameState.shopStock.length === 0 && displayedItems === 0) {
...
function buyItem(itemId) { 
    ...
    if (gameState.purchasedShopItems.includes(itemId)) {
        addLogEntry(`${item.name} a déjà été acquis.`, "info");
        return;
    }

    for (const resource in item.cost) { if (gameState.resources[resource] < item.cost[resource]) { addLogEntry(`Ressources insuffisantes pour acheter ${item.name}.`, "error"); return; } } 
    ...
    if (gameState.shopStock.includes(itemId)) { // Marquer seulement si c'est un item du stock initial "unique"
        gameState.purchasedShopItems.push(itemId);
    }
