            ...
            if (nextLevelDefinition) {
                ...
                if (costObject) {
                    ...
                    for (const resource in costObject) {
                        if (itemsData && itemsData[resource]) { if ((gameState.inventory.filter(invId => invId === resource).length || 0) < costObject[resource]) { canAfford = false; break; }
    ...
    updateBasePreview: function() {
        ...
        for (let r = 0; r < BASE_GRID_SIZE.rows; r++) { for (let c = 0; c < BASE_GRID_SIZE.cols; c++) {
            ...
            if (r === coreRow && c === coreCol) { cell.classList.add('core'); const coreVisual = document.createElement('div'); coreVisual.className = 'base-core-visual'; coreVisual.textContent = 'N'; cell.appendChild(coreVisual); cell.id = 'base-core-visual-cell';
                ...
                } else if (gameState.placementMode.isActive && buildingsData[gameState.placementMode.selectedDefenseType]) { cell.classList.add('placement-active'); cell.title = `Placer ${buildingsData[gameState.placementMode.selectedDefenseType]?.name || 'défense'}`; }}
        ...
        requestAnimationFrame(() => {
            const cells = previewContainer.querySelectorAll('.base-preview-cell'); if (cells.length === 0) return;
            const firstCell = cells[0]; if (!firstCell.offsetWidth && previewContainer.clientWidth === 0) return; 
            const cellClientWidth = firstCell.offsetWidth || (previewContainer.clientWidth / BASE_GRID_SIZE.cols); const cellClientHeight = firstCell.offsetHeight || (previewContainer.clientHeight / BASE_GRID_SIZE.rows);
            const containerStyle = getComputedStyle(previewContainer); const containerPaddingLeft = parseFloat(containerStyle.paddingLeft) || 0; const containerPaddingTop = parseFloat(containerStyle.paddingTop) || 0; const containerBorderLeft = parseFloat(containerStyle.borderLeftWidth) || 0; const containerBorderTop = parseFloat(containerStyle.borderTopWidth) || 0;
            const offsetX = containerPaddingLeft + containerBorderLeft; const offsetY = containerPaddingTop + containerBorderTop;
            const oldVisuals = previewContainer.querySelectorAll('.defense-visual-on-grid, .base-enemy-visual, .boss-visual-dynamic, .enemy-health-bar-on-grid-container, .projectile-visual'); oldVisuals.forEach(v => v.remove());
            for (const instanceId in gameState.defenses) {
                const defenseState = gameState.defenses[instanceId]; const buildingDef = buildingsData[defenseState.id];
                if (defenseState.currentHealth > 0 && buildingDef && defenseState.gridPos) {
                    const defenseVisual = document.createElement('div'); defenseVisual.classList.add('defense-visual-on-grid', defenseState.id); defenseVisual.textContent = `${buildingDef.name.substring(0,1).toUpperCase()}${defenseState.level}`; 
                    defenseVisual.style.position = 'absolute'; defenseVisual.style.width = `${cellClientWidth - 2}px`; defenseVisual.style.height = `${cellClientHeight - 2}px`; defenseVisual.style.display = 'flex'; defenseVisual.style.alignItems = 'center'; defenseVisual.style.justifyContent = 'center'; defenseVisual.style.fontSize = '0.6rem'; defenseVisual.style.boxSizing = 'border-box'; defenseVisual.style.pointerEvents = 'none';
                    ...
                    if (parentCellElement) { defenseVisual.style.left = `${parentCellElement.offsetLeft + 1}px`; defenseVisual.style.top = `${parentCellElement.offsetTop + 1}px`;
                    } else { defenseVisual.style.left = `${(defenseState.gridPos.c * cellClientWidth) + offsetX + 1}px`; defenseVisual.style.top = `${(defenseState.gridPos.r * cellClientHeight) + offsetY + 1}px`; }
                    const healthPercentage = (defenseState.maxHealth > 0 ? (defenseState.currentHealth / defenseState.maxHealth) : 0) * 100;
                    const tempStyleEl = document.createElement('div'); tempStyleEl.className = `defense-visual-on-grid ${defenseState.id}`; document.body.appendChild(tempStyleEl); const computedStyleForType = getComputedStyle(tempStyleEl);
                    ...
                    let borderColor = computedStyleForType.borderColor && computedStyleForType.borderColor !== "rgba(0, 0, 0, 0)" ? computedStyleForType.borderColor : 'var(--border-color)'; let textColor = computedStyleForType.color || 'white'; document.body.removeChild(tempStyleEl);
                    if (healthPercentage < 30) { bgColor = 'rgba(229, 62, 62, 0.8)'; borderColor = '#c53030'; textColor = 'white'; } else if (healthPercentage < 60) { bgColor = 'rgba(236, 201, 75, 0.8)'; borderColor = '#d69e2e'; textColor = 'black'; }
                    defenseVisual.style.backgroundColor = bgColor; defenseVisual.style.border = `1px solid ${borderColor}`; defenseVisual.style.color = textColor; previewContainer.appendChild(defenseVisual);
                }
            }
            if (nightAssaultEnemiesDisplayEl) {
                if (gameState.nightAssault && gameState.nightAssault.isActive && gameState.nightAssault.enemies.length > 0) {
                    gameState.nightAssault.enemies.forEach(enemy => {
                        ...
                        enemyVisual.style.position = 'absolute'; const visualWidth = parseInt(enemyVisual.style.width) || (isBoss ? (enemy.typeInfo.visualSize?.width || 18) : 10); const visualHeight = parseInt(enemyVisual.style.height) || (isBoss ? (enemy.typeInfo.visualSize?.height || 18) : 10);
    ...
    managePlacedDefense: function(instanceId, row, col) {
        ...
        if (canUpgradeInstance) {
            ...
            if (nextLevelData && nextLevelData.costToUpgrade) {
                let costStringForButton = getCostString(nextLevelData.costToUpgrade, true); 
