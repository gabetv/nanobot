...
var explorationUI = {
    ...
    getTileDisplayClass: function(x, y) {
        ...
        if (isNanobotCurrentPos) {
            ...
            switch (actualType) {
                ...
                case TILE_TYPES.EMPTY_WATER: classes.push('water'); break; // Explored shallow water
                case TILE_TYPES.FOREST: classes.push('forest'); break;
                case TILE_TYPES.MOUNTAIN: classes.push('mountain'); break; // Explored standard mountain
                ...
                case TILE_TYPES.IMPASSABLE_DEEP_WATER: classes.push('impassable-water explored-impassable'); break; // Specific class for explored deep water
                case TILE_TYPES.IMPASSABLE_HIGH_PEAK: classes.push('impassable-mountain explored-impassable'); break; // Specific class for explored high peak
                default: classes.push('grassland'); break; 
            }
            // Highlight for active resources on explored tiles
            if (tile.content && tile.content.type === 'resource' && tile.content.amount > 0) {
                classes.push('has-active-resource'); // New class for highlighting
            }
        } else { // Not explored
            ...
            switch (tile.baseType) {
                ...
                case TILE_TYPES.PRE_HIGH_MOUNTAIN: classes.push('unexplored-impassable'); break; // Generic impassable for PRE_HIGH_MOUNTAIN
                case TILE_TYPES.PLAYER_BASE: classes.push('base'); break; // Should be explored from start, but just in case
            ...
            if (tile.isScanned && tile.scannedActualType !== null) {
                ...
                switch(tile.scannedActualType) {
                    ...
                    case TILE_TYPES.IMPASSABLE_DEEP_WATER: classes.push('scanned-actual-impassable-water'); break; // Class for scanned impassable water
                    case TILE_TYPES.IMPASSABLE_HIGH_PEAK: classes.push('scanned-actual-impassable-mountain'); break; // Class for scanned impassable mountain
    ...
    updateExplorationLogDisplay: function() {
        ...
        explorationLogEl.innerHTML = ''; // Clear previous log entries
