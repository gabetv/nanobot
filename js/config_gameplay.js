...
// Assurez-vous que nanobotModulesData (de config_player.js) est défini avant si researchData y fait référence.

var itemsData = {
    'comp_av': { id: 'comp_av', name: "Composant Avancé", slot: null, description: "Matériau de fabrication.", cost: null, rarity: "material" },
    'mod_proto': { id: 'mod_proto', name: "Module Prototypé", slot: null, description: "Prototype de module.", cost: null, rarity: "material"},
    'nanites_medium_cache': { id: 'nanites_medium_cache', name: "Cache Moyenne de Nanites", slot: null, consumable: true, description: "Fournit 150 Nanites.", onUse: (gs) => { gs.resources.nanites = (gs.resources.nanites || 0) + 150; if(typeof addLogEntry === 'function' && typeof eventLogEl !== 'undefined' && gs && gs.eventLog) addLogEntry("Cache de nanites activée: +150 Nanites!", "success", eventLogEl, gs.eventLog); return true; }, cost: null, rarity: "consumable_reward" },
    'item_repair_kit_s': { id: 'item_repair_kit_s', name: "Kit de Réparation (S)", slot: null, consumable: true, description: "Restaure 25 PV.", onUse: (gs) => { if(!gs || !gs.nanobotStats) return false; if(gs.nanobotStats.currentHealth < gs.nanobotStats.health) { gs.nanobotStats.currentHealth = Math.min(gs.nanobotStats.health, gs.nanobotStats.currentHealth + 25); if(typeof addLogEntry === 'function' && typeof eventLogEl !== 'undefined' && gs && gs.eventLog) addLogEntry("Kit de réparation utilisé: +25 PV.", "success", eventLogEl, gs.eventLog); if(typeof uiUpdates !== 'undefined' && typeof uiUpdates.updateNanobotDisplay === 'function') uiUpdates.updateNanobotDisplay(); else if(typeof updateNanobotDisplay === 'function') updateNanobotDisplay(); return true;} else { if(typeof addLogEntry === 'function' && typeof eventLogEl !== 'undefined' && gs && gs.eventLog) addLogEntry("PV du Nanobot déjà au maximum.", "info", eventLogEl, gs.eventLog); return false;} }, cost: {nanites: 50}, rarity: "common" },
    'crist_stock': { id: 'crist_stock', name: "Stock de Cristal", slot: null, description: "Matériau brut.", cost: null, rarity: "material"},
    'crist_stock_large': { id: 'crist_stock_large', name: "Grand Stock de Cristal", slot: null, consumable: true, description: "Fournit 75 Cristaux.", onUse: (gs) => { gs.resources.crystal_shards = (gs.resources.crystal_shards || 0) + 75; if(typeof addLogEntry === 'function' && typeof eventLogEl !== 'undefined' && gs && gs.eventLog) addLogEntry("Vous récupérez un grand stock de cristaux, +75 Cristaux!", "success", eventLogEl, gs.eventLog); return true; }, cost: null, rarity: "consumable_reward"},
    'arte_rare': { id: 'arte_rare', name: "Artefact Rare", slot: null, description: "Relique de valeur.", cost: null, rarity: "rare_material"},
    'crystal_shards_large_cache': {id: 'crystal_shards_large_cache', name: "Grande Cache d'Éclats de Cristal", slot:null, consumable: true, description: "Fournit 200 Éclats de Cristal.", onUse: (gs) => { gs.resources.crystal_shards = (gs.resources.crystal_shards || 0) + 200; if(typeof addLogEntry === 'function' && typeof eventLogEl !== 'undefined' && gs && gs.eventLog) addLogEntry("Grande cache de cristaux: +200 Éclats!", "success", eventLogEl, gs.eventLog); return true;}, cost:null, rarity:"consumable_reward"},
    'frag_alien': {id: 'frag_alien', name: "Fragment Alien", slot: null, description: "Matériau de fabrication.", cost: null, rarity: "material"},
    'item_laser_mk1': { id: 'item_laser_mk1', name: "Laser de Précision Mk1", slot: "weapon", description: "Laser compact.", statBoost: { attack: 8 }, damageType: DAMAGE_TYPES.ENERGY, visualClass: 'item-weapon-laser', cost: { nanites: 150, biomass: 50 }, rarity: "uncommon" },
    'item_nanosword': { id: 'item_nanosword', name: "Épée Nanotech", slot: "weapon", description: "Lame physique.", statBoost: { attack: 10, speed: -1 }, damageType: DAMAGE_TYPES.KINETIC, visualClass: 'module-arm-blade', cost: { nanites: 120, biomass: 80 }, rarity: "uncommon" },
    'item_plating_basic': { id: 'item_plating_basic', name: "Plaquage Standard", slot: "armor", description: "Blindage basique.", statBoost: { defense: 5, health: 10 }, visualClass: 'item-armor-plating', cost: { nanites: 100, biomass: 100 }, rarity: "common" },
    'item_advanced_scope': { id: 'item_advanced_scope', name: "Viseur Avancé", slot: "utility2", description: "Améliore la précision.", statBoost: { attack: 3, speed: -1 }, cost: { nanites: 200 }, rarity: "uncommon" },
};
...
var researchData = { // time est en secondes, sera converti en ticks dans la logique de jeu
    ...
    'weaponizedNanites': { name: "Nanites Armés", description: "Intègre des nanites offensives dans les systèmes du Nexus-7.", cost: { biomass: 400, nanites: 250 }, time: 240, requirements: { buildings: { researchLab: 2 } }, grantsModule: 'armBlade' }, // ID de nanobotModulesData
    'graviticManipulation': { name: "Manipulation Gravitique", description: "Développement de modules de déplacement avancés.", cost: { biomass: 300, nanites: 200, arte_rare: 1 }, time: 180, requirements: { buildings: { researchLab: 2 } }, grantsModule: 'gravLegs' } // ID de nanobotModulesData
};

// --- QUEST_DATA (Déplacé ici car utilise itemsData, researchData, etc.) ---
var QUEST_DATA = {
    'main_01_survival_basics': {
        ...
        unlockConditions: () => true, // Sera vérifié et mis à AVAILABLE par questController
        objectives: [
        ...
        rewards: { xp: 50, items: ['item_repair_kit_s'] }, // itemId de itemsData
        nextQuest: 'main_02_build_harvester' // id de QUEST_DATA
    },
    ...
    'side_01_clear_scouts': {
        ...
        objectives: [ { type: "defeat_enemy_type", enemyId: "drone_scout", count: 3, text: "Éliminer 3 Drones Éclaireurs" } ], // enemyId doit correspondre à une clé de explorationEnemyData
    ...
    'clear_outpost_01': {
        ...
        unlockConditions: (gs) => gs.quests['main_01_survival_basics']?.status === QUEST_STATUS.FINISHED,
        objectives: [ { type: "destroy_enemy_base", baseId: "outpost_alpha", zoneId: "verdant_archipelago", text: "Détruire l'Avant-poste Alpha" } ], // baseId doit être une clé de enemyBaseDefinitions
