...
function initDOMReferences() {
    ...
    combatXpBarEl = document.getElementById('combat-end-modal').querySelector('#xp-bar'); window.combatXpBarEl = combatXpBarEl; // Corrected selector
...
function setupEventListeners() {
    ...
    if(window.modalConfirmBtn) window.modalConfirmBtn.addEventListener('click', () => { if(typeof confirmModal === 'function') confirmModal(); }); // confirmModal just calls hideModal
    ...
    if (simulateCombatButton) {
        simulateCombatButton.addEventListener('click', () => {
            if (typeof window.simulateCombat === 'function' && typeof window.DAMAGE_TYPES !== 'undefined' && typeof window.enemyData !== 'undefined' && window.enemyData.training_drone_01) {
                // Using training_drone_01 from enemyData for consistency
